                ...
                for (const s of sigs.slice(0, 5)) {
                  ...
                  if (dir === "BUY" || dir === "SELL" || dir === "BUY_CANDIDATE" || dir === "SELL_CANDIDATE") {
                    this.listeners.forEach(cb => cb({
                      ...
                      payload: {
                        id: String(s.ID || s.id || ""),
                        direction: (dir === 'BUY' || dir === 'SELL' ? dir : 'NO_TRADE'),
                        ...
                        status: (['ACTIVE','CLOSED','EXPIRED'].includes(String(s.Status || s.status)) ? String(s.Status || s.status) : 'ACTIVE') as 'ACTIVE' | 'CLOSED' | 'EXPIRED',
