...
**Windows Agent Role Split (v1.2.32):** The Windows Agent ships as two roles — **Client Agent** (execution, `pat-agent.exe`, service `pat-agent-client`, engine exec port 13081) and **Master Node** (data-only, `pat-master.exe`, service `pat-agent-master`, engine data port 13091). Both are the same Go build; only the runtime `--mode` and install name/service/port differ. The download server now serves role-specific subfolders (`…/windows-agent/client/` and `…/master/`) so each role's binary is fetched from an unambiguous URL, while shared assets (NSSM, settings, scripts, version) come from the root. See the [Windows Agent Guide](../guides/WINDOWS_AGENT.md).
